...
class _FeedScreenState extends State<FeedScreen> {
  ...
  Future _getMoreFeed() async {
    ...
    QuerySnapshot postDocuments = await query.get();

    if (postDocuments.docs.length < 10) {
      print("Loaded all posts..");
      _canLoadMorePosts = false;
    }

    _postDocuments.addAll(postDocuments.docs);
    _lastDocument = _postDocuments[_postDocuments.length - 1];

    for (int i = 0; i < postDocuments.docs.length; ++i) {
      Card card = _makeCard(postDocuments.docs[i]);
      _posts.add(card);
    }
